...
void MainWindow::createActions () {
    ...
    connect(actZoomMapFull, SIGNAL(triggered()), editorMain, SLOT(zoomMapFull()));
    connect(actZoomMapIn,   SIGNAL(triggered()), editorMain, SLOT(zoomMapIn()));
    connect(actZoomMapOut,  SIGNAL(triggered()), editorMain, SLOT(zoomMapOut()));

    // Show/Hide Grid
    actToggleMapGrid = new QAction(tr("Show Map Grid"), this);
    actToggleMapGrid->setCheckable(true);
    connect(actToggleMapGrid, SIGNAL(triggered(bool)), editorMain, SLOT(toggleMapGrid(bool)));
}
...
bool MainWindow::saveFile (const QString& fileName) {
    ...
    if(!file.open(QFile::WriteOnly)) {
        QMessageBox::warning(this, tr("Nectaris PC Map Editor"),
