...
void EditorPlayers::changePlayerRole (int i) {
    necMapInfo->playerRole[i] = Nec::PlayerRole(combosPlayerRole[i]->currentIndex()).toByte();
}

void EditorPlayers::changePlayerStance (int i) {
    necMapInfo->playerStance[i] = Nec::PlayerStance(combosPlayerStance[i]->currentIndex()).toByte();
}
...
void EditorPlayers::updateNecData () {
    for (int i = 0, ii = combosPlayerStance.size(); i < ii; ++i)
        combosPlayerStance[i]->setCurrentIndex(Nec::PlayerStance(necMapInfo->playerStance[i]).getIndex());
    for (int i = 0, ii = combosPlayerRole.size(); i < ii; ++i)
        combosPlayerRole[i]->setCurrentIndex(Nec::PlayerRole(necMapInfo->playerRole[i]).getIndex());
}
...
QComboBox * EditorPlayers::createPlayerRoleComboBox () {
    ...
    for (int i = 0, ii = Nec::PlayerRole::DATA.size(); i < ii; ++i)
        newComboBox->addItem(QString::fromStdString(Nec::PlayerRole::DATA[i].name));
    return newComboBox;
}
...
QComboBox * EditorPlayers::createPlayerStanceComboBox () {
    ...
    for (int i = 0, ii = Nec::PlayerStance::DATA.size(); i < ii; ++i)
        newComboBox->addItem(QString::fromStdString(Nec::PlayerStance::DATA[i].name));
    return newComboBox;
}
...
void EditorPlayers::createPlayerRoleComboBoxes () {
    ...
    for (int i = 0; i < PLAYERS_COUNT; ++i) {
        ...
        layoutGridPlayerRoles->addWidget(new QLabel("P" + QString::number(i + 1)), 0, i + 1);
...
void EditorPlayers::createPlayerStanceComboBoxes () {
    ...
    // Labels
    for (int i = 0; i < PLAYERS_COUNT; ++i) {
        layoutGridPlayerStances->addWidget(new QLabel("P" + QString::number(i + 1)), 0, i + 1);
        layoutGridPlayerStances->addWidget(new QLabel("P" + QString::number(i + 1)), i + 1, 0);
    }
    ...
    for (int i = 0, k = 0; i < PLAYERS_COUNT; ++i) {
        for (int j = 0; j < PLAYERS_COUNT; ++j, ++k) {
            QComboBox * newComboBoxPlayerStance = createPlayerStanceComboBox();
            layoutGridPlayerStances->addWidget(newComboBoxPlayerStance, i + 1, j + 1);
            connect(
