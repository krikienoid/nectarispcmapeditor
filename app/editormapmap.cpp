...
void EditorMapMap::changeTilesetRange (int) {
    ...
    for (int i = 0; i < TILESETS; ++i)
        pickerTerTile->enabledTilesetRange[i] = checksTilesetRange[i]->isChecked();

    // Nec Data
    for (int i = 0, ii = necMapInfo->bgFiles.size(); i < ii; ++i)
        necMapInfo->bgFiles[i] = 0;
    for (int j = 0, k = 0, kk = necMapInfo->bgFiles.size(); j < TILESETS && k < kk; ++j)
...
void EditorMapMap::loadNecData   (Nec::MapInfo * newNecMapInfo, Nec::MapMap * newNecMapMap) {
...
void EditorMapMap::updateNecData () {
    ...
    viewMapTileGrid->setSceneRect(
        0,
        0,
        Nec::MapMap::getWidth (necMapInfo->quadrantsX + 1) * SceneMapTileGrid::TILE_WIDTH,
        Nec::MapMap::getHeight(necMapInfo->quadrantsY + 1) * SceneMapTileGrid::TILE_HEIGHT
    );
...
void EditorMapMap::updateSelectedTerTile (int terTileNum) {
    ...
    if (terTypeNum < int(Nec::TER_TYPE_DATA.size())) {
        labelTerTileImg->setPixmap(sceneMapTileGrid->pixmapTerTiles->getTerTile(terTileNum));
...
void EditorMapMap::updateEnabledTilesetRange () {
    for (int i = 0; i < TILESETS; ++i)
        checksTilesetRange[i]->setChecked(pickerTerTile->enabledTilesetRange[i]);
...
void EditorMapMap::createMapSizePicker () {
    ...
    connect(comboMapSizeX, SIGNAL(currentIndexChanged(int)), this, SLOT(changeMapSizeX(int)));
    connect(comboMapSizeY, SIGNAL(currentIndexChanged(int)), this, SLOT(changeMapSizeY(int)));
